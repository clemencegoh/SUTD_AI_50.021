...
def run():
  ...
  dtr = torch.utils.data.TensorDataset(torch.from_numpy(xtr).type(torch.FloatTensor),
                                       torch.from_numpy(ytr).type(torch.FloatTensor))
  # TensorDataset from tensors from xv, yv - our validation features and labels
  dv = torch.utils.data.TensorDataset(torch.from_numpy(xv).type(torch.FloatTensor),
                                      torch.from_numpy(yv).type(torch.FloatTensor))
  ...
  for epoch in np.arange(maxnumepochs):
    ...
    for ct,data in enumerate(loadertr):
      ...
      #TODO: done
      model.w.data += -learningrate * model.w.grad.data
      model.bias.data += -learningrate * model.bias.grad.data
